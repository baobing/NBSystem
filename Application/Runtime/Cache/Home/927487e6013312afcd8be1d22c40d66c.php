...
<script>
    $(function() {
        var data={};
        if('<?php echo ($type); ?>'==0){
            data["url"]="/index.php/Home/Query/getListSample/step/3/permission/4";
            data["onDblClickRow"]=function(){
                showCheck();
            }
            data["singleSelect"]=0;
        }else{
            data["url"]="/index.php/Home/Query/getListSample/checked/1/permission/4";
        }
    ...
    function finishTest(){
        ...
        '<?php if(is_array($passer)): $k = 0; $__LIST__ = $passer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>' +
        ...
        $.messager.confirm("请选择批准人员",str,function(data){
            if(data){
                ...
                $.post("<?php echo U(refreshStep);?>",{ids:ids,step:4,pass_person:passer[0]},function(data){
                   if(data) {
